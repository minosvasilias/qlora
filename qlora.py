    ...
    if "llama" in args.model_name_or_path or isinstance(tokenizer, LlamaTokenizer):
        ...
        tokenizer.add_special_tokens({
                ...
                "unk_token": tokenizer.convert_ids_to_tokens(
                    ...
                    if (
                        model.config.pad_token_id != None and 
...
# Default Llama-2 system prompt (from https://github.com/facebookresearch/llama/blob/main/llama/generation.py)
DEFAULT_LLAMA_SYSTEM_PROMPT = """You are a helpful, respectful and honest assistant. Always answer as helpfully as possible, while being safe. Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive in nature.

If a question does not make any sense, or is not factually coherent, explain why instead of answering something not correct. If you don't know the answer to a question, please don't share false information."""

# Prompt templates for different models
LLAMA_PROMPT_TEMPLATE_TEXT_INPUT = "{system_prompt}\n\n### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Response: "
LLAMA_PROMPT_TEMPLATE_CHAT_INPUT = (
    "[INST] <<SYS>>\n{system_prompt}\n<</SYS>>\n\n{instruction}\n{input} [/INST] "
)
LLAMA_PROMPT_TEMPLATE_TEXT = (
    "{system_prompt}\n\n### Instruction:\n{instruction}\n\n### Response: "
)
LLAMA_PROMPT_TEMPLATE_CHAT = (
    "[INST] <<SYS>>\n{system_prompt}\n<</SYS>>\n\n{instruction} [/INST] "
)


def extract_llama_dataset_chat(example):
    return extract_llama_dataset(
        example, LLAMA_PROMPT_TEMPLATE_CHAT, LLAMA_PROMPT_TEMPLATE_CHAT_INPUT
    )


def extract_llama_dataset_text(example):
    return extract_llama_dataset(
        example, LLAMA_PROMPT_TEMPLATE_TEXT, LLAMA_PROMPT_TEMPLATE_TEXT_INPUT
    )


def extract_llama_dataset(example, template, template_input):
    if not "system_prompt" in example.keys():
        example["system_prompt"] = DEFAULT_LLAMA_SYSTEM_PROMPT
    if example.get("input", "") != "":
        prompt_format = template_input
    else:
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def format_dataset(args, dataset, dataset_format):
        ...
        if (
            ...
            if "chat" in args.model_name_or_path:
                dataset = dataset.map(
                    extract_llama_dataset_chat,
                    remove_columns=["instruction", "system_prompt"],
                )
            else:
                dataset = dataset.map(
                    extract_llama_dataset_text,
                    remove_columns=["instruction", "system_prompt"],
                )
    ...
    dataset = format_dataset(args, dataset, args.dataset_format)
